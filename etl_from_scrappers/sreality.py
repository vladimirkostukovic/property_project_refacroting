...
with engine.connect() as conn:
    table_name = conn.execute(text("""
        ...
        WHERE table_schema = 'public' AND table_name LIKE 'sreality\\_%' ESCAPE '\\'
...
with engine.begin() as conn:
    ...
    if not df_standart.empty:
        ...
        df_standart.to_sql('standartize', con=conn, schema='public', if_exists='append', index=False)
...
try:
    ...
    if yesterday_exists and not price_snapshot.empty:
        try:
            # Load yesterday's prices (only internal_id, price)
            ...
            if not price_yesterday.empty:
                # Merge today's and yesterday's prices on internal_id
...
with engine.begin() as conn:
    # Ensure main photo links table exists
    conn.execute(text("""
...
# Load active listings and existing photo links once
active_stand = pd.read_sql("""
    SELECT internal_id, site_id 
    FROM public.standartize 
    WHERE source_id = 1 AND archived_date IS NULL
""", con=engine)
...
for idx, row in active_stand.iterrows():
    sid = row['site_id']
    iid = row['internal_id']

    images = site_images_map.get(sid)
    if not images:
        continue
    ...
    try:
        ...
        if isinstance(img_list, list):
            for img in img_list:
                ...
                if url and (iid, url) not in existing_set:
                    photo_links.append({
                        'internal_id': iid,
                        'site_id': sid,
                        'photo_url': url,
        ...
        if processed_count % 100 == 0:
            log.info(f"  → Processed {processed_count} listings for photo links extraction...")
    except Exception as e:
        log.warning(f"Photo links parse failed for site_id={sid}: {e}")

if photo_links:
    log.info(f"Total new photo links to insert: {len(photo_links)}")
    df_photos = pd.DataFrame(photo_links)
    with engine.begin() as conn:
        # Create temp table to hold new photo links (lives during this transaction)
        conn.execute(text("""
            CREATE TEMP TABLE tmp_photo_links (
                internal_id INTEGER,
                site_id TEXT,
                photo_url TEXT,
                added_date TIMESTAMP,
        ...
        # Insert all new photo links into temp table
        df_photos.to_sql('tmp_photo_links', con=conn, if_exists='append', index=False)

        # Insert unique photo links from temp table into main table
        conn.execute(text("""
            INSERT INTO public.sreality_links_photo (internal_id, site_id, photo_url, added_date, source_id)
            SELECT tmp.internal_id, tmp.site_id, tmp.photo_url, tmp.added_date, tmp.source_id
            FROM tmp_photo_links tmp
            LEFT JOIN public.sreality_links_photo exist
              ON tmp.internal_id = exist.internal_id AND tmp.photo_url = exist.photo_url
            WHERE exist.internal_id IS NULL;
        """))
    stat_row("New photo links added", len(photo_links))
else:
